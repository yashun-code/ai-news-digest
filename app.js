(() => {
  ...
  const state = { index: null, digests: new Map(), toastTimer: null };
  ...
  function parseDigest(markdown) {
    ...
    for (; cursor < lines.length; cursor += 1) {
      ...
      if (line.startsWith('## ')) {
        ...
        item = { title: line.slice(3).trim(), summaryLines: [], term: '', relate: '', sources: [] };
      } else if (item) {
        if (line.startsWith('- term:')) item.term = line.slice(7).trim();
  ...
  function finishCopy(button, succeeded) {
    if (succeeded) {
      button.textContent = '✓ コピー完了';
      button.classList.add('done');
      showToast('コピーしました。ChatGPTやClaudeなどのAIに貼り付けると詳しく教えてもらえます');
    } else {
  ...
  function copyPrompt(button, item) {
    ...
    if (navigator.clipboard && typeof navigator.clipboard.writeText === 'function') {
      navigator.clipboard.writeText(text)
        .then(() => finishCopy(button, true))
        .catch(() => finishCopy(button, legacyCopy(text)));
    } else {
      finishCopy(button, legacyCopy(text));
    }
  }
  ...
  function renderMenu() {
    ...
    app.innerHTML = `
      ...
      <div class="caption"><span aria-hidden="true">📅</span><span>3日に1回 自動で更新。ホーム画面から最新のダイジェストを確認できます。</span></div>`;
  }
